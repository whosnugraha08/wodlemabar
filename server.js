...
function getOrCreateRoom(roomId = "daily") {
  if (!rooms[roomId]) {
    ...
    rooms[roomId] = {
      id: roomId,
      answer: isDailyRoom ? getDailyWord() : ANSWER_WORDS[Math.floor(Math.random() * ANSWER_WORDS.length)],
...
function broadcastRoomFinished(room) {
  console.log(`🏁 Room "${room.id}" finished!`);
  ...
  socket.on("disconnect", (reason) => {
    ...
    if (remaining.length === 0) {
      if (currentRoom === "daily") {
        // Keep the room but reset status so next person can play
        room.status = "playing";
        console.log(`🏠 Daily room now empty, kept alive`);
      } else {
